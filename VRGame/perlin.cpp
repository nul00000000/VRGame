...
double Perlin::noise(double x, double y, double z) {
	...
	double rx0, rx1, ry0, ry1, rz, sx, sy, sz, a, b, c, d, u, v;
	double* q;

	bx = (int)std::remainder(std::floor(x), B);
	...
	by = (int)std::remainder(std::floor(y), B);
	if (by < 0)
		by += B;
	ry0 = y - std::floor(y);
	ry1 = ry0 - 1;

	bz = (int)std::remainder(std::floor(z), B);
	if (bz < 0)
		bz += B;
	rz = z - std::floor(z);
...
Perlin::Perlin(int seed) {
	...
	for (i = 0; i < B; i++) {
		p[i] = i;
		g1[i] = 2 * (((int)r() % 1000) / 1000.0) - 1;

		do {
			u = 2 * (((int)r() % 1000) / 1000.0) - 1;
			v = 2 * (((int)r() % 1000) / 1000.0) - 1;
		} while (u * u + v * v > 1 || std::abs(u) > 2.5 * std::abs(v) || std::abs(v) > 2.5 * std::abs(u) || std::abs(std::abs(u) - std::abs(v)) < .4);
		g2[i][0] = u;
		g2[i][1] = v;
		normalize2(g2[i]);
		...
		do {
			u = 2 * (((int)r() % 1000) / 1000.0) - 1;
			v = 2 * (((int)r() % 1000) / 1000.0) - 1;
			w = 2 * (((int)r() % 1000) / 1000.0) - 1;
			U = std::abs(u);
			V = std::abs(v);
			W = std::abs(w);
			Lo = std::min(U, std::min(V, W));
			Hi = std::max(U, std::max(V, W));
		} while (u * u + v * v + w * w > 1 || Hi > 4 * Lo || std::min(std::abs(U - V), std::min(std::abs(U - W), std::abs(V - W))) < .2);
	}
	...
	points[3][0] = points[3][1] = points[3][2] = std::sqrt(1. / 3);
	double r2 = std::sqrt(1. / 2);
	double s = std::sqrt(2 + r2 + r2);
...
void Perlin::normalize2(double* v) {
	double s;
